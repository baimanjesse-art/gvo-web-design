...
const TEAM = [
  {
    ...
    // PLACEHOLDER: drop the real headshot at /public/images/team/jesse.jpg — the card picks it up automatically.
  ...
  },
  {
    ...
    // PLACEHOLDER: drop the real headshot at /public/images/team/elliott.jpg — the card picks it up automatically.
...
function TeamPhoto({ member }) {
  ...
  if (failed) {
    // Fallback avatar shown until the real headshot is added to /public/images/team/
    return (
...
export default function About() {
  return (
    <section id="about" className="scroll-mt-16 bg-ink-900 px-4 py-24 sm:px-6 md:py-32">
      <div className="mx-auto max-w-6xl">
        <SectionHeading
          eyebrow="About Us"
          title="Two builders. One standard."
          sub="GVO is a two-person shop by design — you talk directly to the people doing the work, and nothing ships until it's right."
        />

        <div className="grid gap-8 md:grid-cols-2">
          {TEAM.map((member, i) => (
            <Reveal key={member.name} delay={i * 0.12}>
              <article className="flex h-full flex-col items-center rounded-2xl border border-white/8 bg-ink-800 p-8 text-center shadow-[inset_0_1px_0_rgba(255,255,255,0.05)] md:p-10">
